            ...
            <thead>
              <tr className="text-xs text-gray-600 uppercase tracking-wider border-b border-gray-100">
                {["ID", "Donante", "Tipo", "Cantidad", "Fecha", "Estado", "Avanzar estado"].map(
                  (heading) => (
                    <th key={heading} scope="col" className="text-left pb-3 font-medium">{heading}</th>
                  )
                )}
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {filtered.map((donation) => (
                <tr key={donation.id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="py-4">
                    {getNextStatus(donation.estado) ? (
                      <button
                        type="button"
                        onClick={() => handleAdvanceState(donation.id)}
                        aria-label={`Avanzar la donacion ${donation.id} de "${donation.estado}" a "${getNextStatus(donation.estado)}"`}
                      ...
                      >
                        Avanzar a {getNextStatus(donation.estado)}
                      </button>
                    ) : (
